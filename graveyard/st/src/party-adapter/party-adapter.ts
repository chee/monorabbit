...
import {ProtocolV1, ProtocolVersion} from "./protocol-version"
import {assert} from "./assert"
// import { handleChunked, sendChunked } from './chunking.js';

const {encode, decode} = cborHelpers

export class PartyServerAdapter extends NetworkAdapter {
	#isReady = false
	#readyPromise
	#readyResolver
	...
	send(message: FromServerMessage) {
		...
		this.socket.send(arrayBuf, {binary: true, compress: false})
	}
